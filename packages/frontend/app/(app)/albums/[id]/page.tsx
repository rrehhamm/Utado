      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-10 flex flex-col items-center gap-8 sm:flex-row sm:items-start">
          ...
          <div>
            ...
            <dl className="mt-6 grid grid-cols-2 gap-x-8 gap-y-2 text-sm text-ink/70">
              <div>
                <dt className="text-ink/40">Genre</dt>
                <dd>{album.genre ?? "—"}</dd>
              </div>
              <div>
                <dt className="text-ink/40">Released</dt>
        ...
        <div className="mt-12">
          <h2 className="mb-4 text-lg font-bold text-ink">Tracklist</h2>
          <ul className="divide-y divide-ink/10 rounded-xl2 bg-surface-elevated/60 shadow-soft">
            {album.songs.map((song, i) => (
              <li key={song.id}>
                <Link
                  href={`/songs/${song.id}`}
                  className="flex items-center justify-between px-6 py-4 hover:bg-surface-elevated"
                >
                  <span className="flex items-center gap-4">
                    <span className="w-5 text-sm text-ink/30">{i + 1}</span>
                    <span className="font-medium text-ink">{song.title}</span>
                  </span>
                  <span className="text-sm text-ink/40">{formatDuration(song.duration)}</span>
                </Link>
              </li>
            ))}
          </ul>
